"""Helper for full Mojo GPU denoiser — renders Mitsuba scenes, returns RGB GT target."""
...
def render_tile_full(scene_idx, seed, tile_size, sf_dim):
    """Render scene with real AOVs. Returns dict with aov, sf, gt_rgb arrays."""
    scene, sg = _BUILDERS[scene_idx % 2](resolution=(tile_size, tile_size))

    # Clean GT (high SPP) — this is the RGB target
    gt = np.array(mi.render(scene, spp=64, seed=0))[:, :, :3].astype("float32")
    gt_rgb = gt.reshape(-1).astype("float32")
    ...
    aov = np.concatenate(
        [albedo, normal, depth, position, uv, mat, px, py], axis=-1
    ).astype("float32")
    ...
    sf = (
        np.concatenate(parts).astype("float32")
        if parts
        else np.zeros(sf_dim, dtype="float32")
    )
    if sf.shape[0] < sf_dim:
        sf = np.pad(sf, (0, sf_dim - sf.shape[0]))
    sf = sf[:sf_dim].astype("float32")

    return {"aov": aov.ravel(), "sf": sf, "gt_rgb": gt_rgb}
