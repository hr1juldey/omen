...
- [x] 3.1 Create `src/omen/kernels/conv2d.py` — `Im2colOp(Operation)` wrapping the Mojo im2col kernel. `compute_physical_shape` returns `[(B·H_out·W_out, K·K·C_in)]`. `kernel()` calls `call_custom_kernel("conv2d_im2col", ...)`.
- [x] 3.2 Add `Col2imOp(Operation)` in same file — wraps Mojo col2im kernel. `compute_physical_shape` returns `[(B, H, W, C_in)]`. `kernel()` calls `call_custom_kernel("conv2im_col2im", ...)`.
- [x] 3.3 Add `Conv2dOp(Operation)` — `compute_physical_shape` returns output NHWC shape. `kernel()` calls `Im2colOp` to get patches, then `nb.matmul(patches, filter_flat)`, then reshape to `(B, H_out, W_out, C_out)`.
- [x] 3.4 Add `Conv2dOp.vjp_rule()` — pure Nabla ops: `im2col(x).T @ cot_flat` for grad_filter, `col2im(cot_flat @ filter_flat.T)` for grad_input. Returns `[grad_x, grad_w]`.
- [x] 3.5 Add `conv2d_safe(x, filter, stride=1, padding=0, bias=None)` wrapper: normalize stride/padding to tuples, dispatch to Conv2dOp, add bias if provided.
- [x] 3.6 Add `conv2d_safe` export to `src/omen/kernels/__init__.py`.
...
- [x] 5.1 Replace 8 `nb.conv2d` calls in `src/omen/model/decoder.py` with `conv2d_safe` — import from `omen.kernels`, preserve all stride/padding args.
- [x] 5.2 Replace 3 `nb.conv2d` calls in `src/omen/model/scene_encoder.py` `RenderFeatureEncoder.forward` with `conv2d_safe`.

## 6. Clean Up Trainer

- [x] 6.1 Delete `CONV2D_BLOCKERS` frozenset from `src/omen/training/trainer/core.py`.
- [x] 6.2 Delete `_to_real` helper function.
- [x] 6.3 Simplify `_realize_grads` — remove conv2d blocker branch.
- [x] 6.4 Simplify `_apply_optimizer_updates` — remove `_to_real` calls on optimizer outputs.
