...
def _mla_compress(features, down_w, down_b):
    return nb.silu(features @ down_w + down_b)
...
def decoder_fn(p, latent, noisy_image):
    ...
    """
    # Encoder path
    s1 = nb.silu(conv2d_safe(noisy_image, p["e1"], padding=(1, 1)))
    s2 = nb.silu(conv2d_safe(s1, p["e2"], stride=(2, 2), padding=(1, 1)))
    s3 = nb.silu(conv2d_safe(s2, p["e3"], stride=(2, 2), padding=(1, 1)))
    e4 = nb.silu(conv2d_safe(s3, p["e4"], stride=(2, 2), padding=(1, 1)))

    # Bottleneck: gated JEPA latent injection
    gate = nb.sigmoid(_linear(latent, p["lat_gate.weight"], p["lat_gate.bias"]))
